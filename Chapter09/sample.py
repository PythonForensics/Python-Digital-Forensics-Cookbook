    ...
    if dir_obj_test4 is not None:
        for dir_tuple in dir_obj_test4:
            for fs_object in dir_tuple[1]:
                ...
                file_path = "PARTITION {}{}/{}".format(
                    dir_tuple[0], "/WINDOWS/SYSTEM32/CONFIG", file_name)
                print("Found File in Dir: {}".format(file_path))

    # This will scan from the specified dir (root in this case)
    # and is equal to the word "SAM" and is case sensitive

    file_obj_test2 = tsk_util.recurse_files(
        "SAM", path="/", equal=True, case=True)
...
if __name__ == "__main__":
    ...
    if (os.path.exists(args.EVIDENCE_FILE) and
        ...
        print("[-] Supplied input file {} does "
              "not exist or is not a file".format(args.EVIDENCE_FILE))
        sys.exit(1)
